...
public class Issac {
	...
	// 것을 방지하기 위해서 synchronized (동기화 유지 시켜줘야함)
	...
	public synchronized void buyToast (String name, int count) {
		...
		toast-=count;
		System.out.println(name +"에게 토스트를" + count + "께 팔았습니다.");
		orderList.remove(); // 처리된 주문제거(큐 구조에서 처음께 삭제됨)
		notifyAll(); //다음 대기중인 주문 처리를 위해 모든 스렏 깨움.
}
	}
